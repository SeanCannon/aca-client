...
class RenderTarget {
  ...
  drawPixel(x, y, color) {
    ...
    if (typeof this.opt.drawCallback === 'function') {
...
function basicDrawing(x, y, tool) {
  tool.drawPixel(x, y);
}
...
class DrawingTool {
  ...
  pushRedo() {
    ...
    if (this.redoHistory.length > 25) {
      this.redoHistory.splice(1, this.redoHistory.length - 25);
    }
  }
  ...
  undo() {
    ...
    this.pushRedo();
    const pState = this.undoHistory.pop();
    for (let i = 0; i < 4096; ++i) {
      this.pixels[i] = pState.pixels[i];
    }
    for (let i = 0; i < 15; ++i) {
      this.pattern.setPalette(i, pState.palette[i]);
    }
    this.onColorChange();
    this.render();
    return true;
  ...
  redo() {
    ...
    this.pushUndo();
    const pState = this.redoHistory.pop();
    for (let i = 0; i < 4096; ++i) {
      this.pixels[i] = pState.pixels[i];
    }
    for (let i = 0; i < 15; ++i) {
  ...
  get fullHash() {
    return this.pattern.fullHash();
  }

  get pixelHash() {
    return this.pattern.pixelHash();
  }

  get title() {
    return this.pattern.title;
  }

  set title(n) {
    this.pattern.title = n;
  }

  get creator() {
    return this.pattern.creator;
  }

  set creator(n) {
    this.pattern.creator = n;
  }

  get town() {
    return this.pattern.town;
  }
  ...
  fixIssues() {
    this.pattern.fixIssues();
  }

  get authorStrict() {
    let copiedCreator = '';
    for (let i = 0; i < 44; ++i) {
      copiedCreator += String.fromCharCode(this.pattern.dataBytes[0x2A + i]);
    }
  ...
  set authorStrict(n) {
    for (let i = 0; i < 44; ++i) {
      this.pattern.dataBytes[0x2A + i] = n.charCodeAt(i);
    }
  }

  get patternType() {
    return this.pattern.patternType;
  }
  ...
  findRGB(c) {
    ...
    let bestno = 0;
    for (let i = 0; i < 256; i++) {
      const m = ACNLFormat.RGBLookup[i];
      if (m === null) {
        continue;
      }
      const rD = (m[0] - rgb[0]);
      ...
      if (match < best) {
        if (!match) {
          return i;
        }// perfect match - we can stop immediately
        best = match;
        bestno = i;
      }
    }
  ...
  findPalRGB(c) {
    ...
    let bestno = 0;
    for (let i = 0; i < 15; i++) {
      const m = ACNLFormat.RGBLookup[this.pattern.getPalette(i)];
      if (m === null) {
        continue;
      }
      const rD = (m[0] - rgb[0]);
  ...
  findYUV(rgb) {
    // Convert to YUV
    const yuv = [rgb[0] * 0.299000 + rgb[1] * 0.587000 + rgb[2] * 0.114000, rgb[0] * -0.168736 + rgb[1] * -0.331264 + rgb[2] * 0.500000 + 128, rgb[0] * 0.500000 + rgb[1] * -0.418688 + rgb[2] * -0.081312 + 128];
    // Find the closest match
    let best = 255 * 255 * 3;
    let bestno = 0;
    for (let i = 0; i < 256; i++) {
      const m = ACNLFormat.YUVLookup[i];
      if (m === null) {
        continue;
      }
      const yD = (m[0] - yuv[0]);
  ...
  handleCanvasClick(e) {
    ...
    const x = Math.floor(xpos / (e.target.width / this.pattern.width));
    const y = Math.floor(ypos / (e.target.height / this.pattern.width));
    if (e.which === 1) {
      this.drawHandler(x, y, this);
    }
    if (e.which === 3) {
      this.drawHandlerAlt(x, y, this);
    }
  }

  // / Adds a canvas to the internal list of render targets.
  addCanvas(c, opt = {}) {
    const rTarget = new RenderTarget(c, opt);
    ...
    c.addEventListener('mousemove', e => {
      if (this.drawing && (e.which === 1 || e.which === 3)) {
  ...
  setPixel(x, y, color = null) {
    ...
    if (x < 0 || y < 0 || color < 0 || color > 15 || x > 63 || y > 63 || isNaN(x) || isNaN(y)) {
      return false;
    }
    if (this.pattern.width === 32 && (x > 31 || y > 31)) {
      return false;
    }
    if (x > 31) {
      x -= 32;
    ...
    const offset = x + y * 32;
    if (this.pixels[offset] === color || this.pixels[offset] === 0xFC) {
      return false;
    }
  ...
  drawPixel(x, y, color = null) {
    ...
    const htmlColor = ACNLFormat.paletteColors[this.pattern.getPalette(color)];
    for (const i in this.renderTargets) {
  ...
  onLoad(f = null) {
    if (f === null) {
      for (const i in this.handleOnLoad) {
  ...
  onColorChange(f = null) {
    if (f === null) {
      for (const i in this.handleColorChange) {
