...
const maybeCreateTenantAccessPermission = data => (dispatch, getState) => {
  ...
  return fetch(url, makeCommonFetchOptions(fetchOptions))
    .then(jsonOrError('Grant permission to an access role'))
    .then(res => {
...
export default maybeCreateTenantAccessPermission;
